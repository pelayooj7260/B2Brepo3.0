...
export default function AuditPricingPage() {
  ...
  const [paymentStatus, setPaymentStatus] = useState<'idle' | 'cancel'>('idle');

  // Activate scroll tracking hook
  useScrollDepthTracker('Business Infrastructure Audit Pricing Page');

  // Sync CMS version and check Stripe payment redirects & prefilled parameters
  useEffect(() => {
    ...
    if (size === '1-10' || size === '11-50' || size === '50+') {
      setBusinessSize(size);
      if (size === '1-10') setSelectedTier('starter');
      else if (size === '11-50') setSelectedTier('full');
      else if (size === '50+') setSelectedTier('enterprise');
    }
